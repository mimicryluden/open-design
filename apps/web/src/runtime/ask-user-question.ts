 ...
 *     live streaming pass, where the input is a JSON *prefix* that grows one
 *     token at a time. It repairs the truncated prefix into the largest
 *     valid value it can, then keeps every question that already has its
 *     prompt text (even with zero options yet) so a question doesn't flicker
 *     in → out → back in as its options arrive.
 *
...
// Re-exported so existing importers/tests of these generic helpers keep
// working after they moved to the shared `partial-json` module.
...
/** Marker the streaming pass stamps on synthesized partial input so the card
 *  knows to render read-only (no selection, no submit) and play its reveal.
 *  The final `tool_use` from the daemon carries real input *without* it. */
export const AUQ_STREAMING_KEY = '__streaming';

export function isStreamingAuqInput(input: unknown): boolean {
  return (
    !!input &&
    typeof input === 'object' &&
    (input as Record<string, unknown>)[AUQ_STREAMING_KEY] === true
  );
}
 ...
 * buffer* (fragments concatenated), repairs the truncation, and shapes it —
 * keeping any question that already has prompt text even if its options have
 * not arrived. The card renders these read-only until the final tool_use
 * supersedes them.
 */
...
/** Shape an already-parsed (lenient) object into questions. Exposed so the
 *  card can re-shape synthesized streaming input without re-parsing. */
